                ...
                <div class="statbox widget box box-shadow">
                    ...
                    <div class="widget-content widget-content-area ">
                        ...
                        <div class="table-responsive mb-4 mt-4">
                            <table id="table_surat_eskul" class="table table-bordered table-striped mb-4" style="width:100%">
                                <thead>
                                    <tr>
...
<!--  MODAL SINGLE VIEW  -->
<?php echo form_open_multipart(current_url(), 'class="form-horizontal" id="form_tambah_surat_eskul"');  ?>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input id="format_tanggal" name="tanggal_surat_eskul" class="form-control flatpickr flatpickr-input active" type="text" placeholder="Isi Tanggal..">
                        </div>
                    </div>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="hidden" class="form-control" style="font-size: 0.85rem!important;" name="tahun_surat_eskul" placeholder="PERIHAL SURAT" value="<?php echo date("Y"); ?>">
                    <input type="hidden" class="form-control" style="font-size: 0.85rem!important;" name="pembuat_surat_eskul" value="<?= $user_info['id_unik']; ?>">
...
<div class="modal fade view_modal_data" id="view_modal_data" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" keyboard="false">
    <div class="modal-dialog" role="document" id="modal-edit">
        <div class="modal-content">
            ...
            <?php echo form_open(base_url('/akun/app/arsipsurat/simpan_edit_surat_eskul'), 'class="form-horizontal" id="form_edit_surat_eskul"');  ?>
            <div class="modal-body load_modal_view">
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-dark mb-2" data-dismiss="modal">Tutup</button>
    ...
    <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <input type="hidden" name="penghapus_surat_eskul" class="form-control" style="font-size: 0.85rem!important;" id="penghapus_surat_eskul" value="<?= $user_info['id_unik']; ?>">
...
<script>
    $(document).ready(function() {
        ...
        var list_surat_eskul = $("#table_surat_eskul").DataTable({
            ...
            ajax: {
                <?php if ($user_info['is_admin'] == 1) { ?> "url": "get_all_surat_eskul_admin/<?php echo date('Y') ?>",
                <?php } else { ?> "url": "get_all_surat_eskul/<?php echo date('Y') ?>",
                <?php } ?> "dataSrc": "",
            },
            columns: [{
                    orderable: true,
                ...
                {
                    orderable: false,
                    data: "second_surat_eskul"
                },

                {
                    orderable: false,
                    data: "no_berkas_eskul"
                },
                {
                    orderable: false,
                    data: "perihal_surat_eskul"
                },
                {
                    orderable: false,
                    data: "alamat_surat_eskul"
                },
                {
                    orderable: false,
                    data: "tanggal_surat_eskul"
                },
                    ...
                    data: function(row, type, set) {
                        if (<?= $user_info['is_admin']; ?> == 1) {
                            if (row.deleted_at == '0000-00-00 00:00:00') {
                                return (
                                    '<div class="btn-group">' +
                                    ' <button style="margin-right: 0px;" type="button" class="btn btn-dark btn-sm" onclick="modal_edit_surat_eskul(' + row.surat_eskul_id + ')" data-toggle="modal" data-target=".view_modal_data">Lihat</button>' +
                            ...
                            if (row.pembuat_surat_eskul == "<?= $user_info['id_unik']; ?>") {
                                return (
                                    '<div class="btn-group">' +
                                    ' <button style="margin-right: 0px;" type="button" class="btn btn-dark btn-sm" onclick="modal_edit_surat_eskul(' + row.surat_eskul_id + ')" data-toggle="modal" data-target=".view_modal_data">Lihat</button>' +
                                    ' <button type="button" class="btn btn-dark btn-sm dropdown-toggle dropdown-toggle-split" id="dropdownMenuReference27" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" data-reference="parent">' +
        ...
        list_surat_eskul.order([0, "desc"], [1, "desc"]).draw();

        $("#list_surat_eskul").on("click", ".editSurat", function() {
        ...
        $("#list_surat_eskul").on("click", ".deleteRecord", function() {
            var UserId = $(this).data("id");
            var nama_data = $(this).data("nama");
        ...
        $("#deleteUserForm").on("submit", function() {
            ...
            var penghapus_surat_eskul = $("#penghapus_surat_eskul").val();
            var token = $('input[name="csrf_token_web"]').attr("value");
            $.ajax({
                type: "POST",
                url: "hapus_surat_eskul",
                dataType: "JSON",
                data: {
                    id: UserId,
                ...
                success: function(data) {
                    if (data == false) {
                        ...
                        $("#deleteUserId").val("");
                        $("#deleteUserModal").modal("hide");
                        list_surat_eskul.ajax.reload(function() {});
                    }
                },
            });
            return false;
        ...
        var form_tambah_surat_eskul = $("#form_tambah_surat_eskul").attr("action");
        $("#form_tambah_surat_eskul").submit(function(e) {
            ...
            $.ajax({
                ...
                success: function(result) {
                    ...
                    if (result == 1) {
                        ...
                        $("#form_tambah_surat_eskul")[0].reset();
                        list_surat_eskul.ajax.reload(function() {});
                        Swal.fire({
                            position: "top-end",
                            type: "success",
        ...
        var form_edit_surat_eskul = $("#form_edit_surat_eskul").attr("action");
        $("#form_edit_surat_eskul").submit(function(e) {
            e.preventDefault();
            var formData = new FormData(this);
            $.ajax({
                url: form_edit_surat_eskul,
                type: "post",
                data: formData,
                processData: false,
                contentType: false,
...
<script type="text/javascript">
    function modal_edit_surat_eskul(id) {

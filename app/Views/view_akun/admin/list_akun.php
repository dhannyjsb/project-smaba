...
<script>
    var list_siswa = $('#table_siswa').DataTable({
        ...
        ajax: {
            url: "<?php echo base_url(); ?>/akun/admin/users/get_all_gtkptk",
            dataSrc: "",
        },
        columns: [{
                data: null
            ...
            },
            {
                data: function(row, type, set) {
                    if (row.user_role_id == 1) {
                        if (row.email_guru == null) {
                            return ("<span class='shadow-none badge list_suspend'>Tidak</span>")
                        } else {
                            return ("<span class='shadow-none badge list_active'>Ada</span>")
                        }
                    }
                    if (row.user_role_id == 2) {
                        if (row.email_tendik == null) {
                            return ("<span class='shadow-none badge list_suspend'>Tidak</span>")
                        } else {
                            return ("<span class='shadow-none badge list_active'>Ada</span>")
                        }
            ...
            },
            {
                data: function(row, type, set) {
                    if (row.is_active == 1) {
                        return ("<span class='shadow-none badge list_active'>Active</span>")
                    } else {
                        return ("<span class='shadow-none badge list_suspend'>Suspend</span>")
            ...
            },
            {
                data: function(row, type, set) {
                    if (row.is_sync == 1) {
                        return ("<span class='shadow-none badge list_active'><svg width='17' height='17' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' class='feather feather-check-circle'><path d='M22 11.08V12a10 10 0 1 1-5.93-9.14'></path><polyline points='22 4 12 14.01 9 11.01'></polyline></svg></span>")
                    } else {
                        return ("<span class='shadow-none badge list_suspend'><svg width='17' height='17' viewBox='0 0 24 24' fill='none' stroke='currentColor' stroke-width='2' stroke-linecap='round' stroke-linejoin='round' class='feather feather-alert-circle'><circle cx='12' cy='12' r='10'></circle><line x1='12' y1='8' x2='12' y2='12'></line><line x1='12' y1='16' x2='12.01' y2='16'></line></svg></span>")
            ...
            {
                data: function(row, type, set) {
                    var a = new Date(row.last_login * 1000);
            ...
            },
            {
                data: function(row, type, set) {
                    return (
                        '<div class="btn-group">' +
                        ' <button type="button" class="btn btn-dark btn-sm" onclick="lihat_modal_siswa(' + row.user_id + ',' + row.user_role_id + ')" data-toggle="modal" data-target=".view_modal_siswa">Lihat</button>' +
...
<script type="text/javascript">
    function lihat_modal_siswa(id, role) {
        ...
        $.ajax({
            ...
            url: "<?php echo base_url($request->uri->getSegment(1) . '/admin/users/single_users'); ?>",
            data: dataJson,
